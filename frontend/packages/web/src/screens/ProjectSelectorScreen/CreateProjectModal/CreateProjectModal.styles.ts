export const useCreateProjectModalStyles = () => ({
    // z-[60] clears the fixed FloatingNav (z-50) — see TaskDetailPanel.styles.
    overlay: 'fixed inset-0 z-[60] flex items-end justify-center bg-black/50 backdrop-blur-sm sm:items-center',
    panel: 'max-h-[90dvh] w-full max-w-md overflow-y-auto rounded-t-2xl border border-border bg-surface-overlay ' +
        'px-6 pt-6 pb-[calc(1.5rem+env(safe-area-inset-bottom))] sm:rounded-2xl sm:pb-6 ' +
    ...
    typeButtonBase: 'min-h-11 flex-1 rounded-lg border py-1.5 text-sm font-medium transition-colors sm:min-h-0',
    ...
    colorSwatchBase: 'h-9 w-9 overflow-hidden rounded-full transition-transform sm:h-7 sm:w-7',
